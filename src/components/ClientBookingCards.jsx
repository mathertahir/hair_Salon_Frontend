...
const ClientBookingCards = ({ bookings = [] }) => {
    return (
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
            {bookings.map((booking) => (
                <div key={booking.id} className='flex-1 flex flex-col'>
                    <div className='lg:h-[275px] h-[200px] rounded-t-3xl'>
                        <img src={booking.image} alt="hairstylist" className='w-full h-full object-cover rounded-t-3xl' />
                    </div>
                        ...
                        <div className='p-6 border border-gray-55 rounded-b-3xl flex-1 flex flex-col'>
                            <div className='flex justify-between items-center'>
                                <div className='text-brown-A43 flex items-center gap-1'>
                                    ...
                                    <span className='text-base font-manrope font-normal text-brown-A43'>{booking.rating}</span>
                                </div>
                                <p className='text-lg font-playfair font-bold text-black-14'>{booking.status === 'Completed' ? 'Completed' : booking.bookingId}</p>
                            </div>

                            <div className='flex flex-col gap-4 flex-1'>
                                <div className='flex-1 flex flex-col items-start gap-[10px]'>
                                    <p className='text-lg font-playfair font-bold text-black-14'>{booking.name}</p>
                                ...
                                <Link to={`/client-booking-info/${booking.id}`}>
                                    <ButtonSquare
                                        variant='outline'
...
export default ClientBookingCards