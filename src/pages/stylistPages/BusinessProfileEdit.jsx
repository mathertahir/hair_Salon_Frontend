...
const BusinessProfileEdit = () => {
  ...
  const handleRemoveImage = (type, id) => {
    ...
    setBusinessProfile((prev) => {
      ...
      if (
        typeof prev[updateKey] === "object" &&
        !Array.isArray(prev[updateKey])
      ) {
        return {
          ...prev,
          [updateKey]: null,
  ...
  const buildFormData = () => {
    ...
    formData.businessPhotos?.forEach((f) =>
      payload.append("businessPhotos", f)
    );
    formData.businessNICPhoto?.forEach((f) =>
      payload.append("businessNICPhoto", f)
    );
    if (formData.businessRegistrationDoc?.[0])
      payload.append(
        "businessRegistrationDoc",
        formData.businessRegistrationDoc[0]
      );
    if (formData.businessFeaturedImage?.[0])
      payload.append(
        "businessFeaturedImage",
        formData.businessFeaturedImage[0]
      );

    // ✅ Add removal arrays (once only)
    payload.append(
      "removeBusinessPhotosIds",
      JSON.stringify(parseIdArray(formData.removeBusinessPhotosIds))
    );
    payload.append(
      "removeBusinessNICPhotoIds",
      JSON.stringify(parseIdArray(formData.removeBusinessNICPhotoIds))
    );
    payload.append(
      "removeBusinessRegistrationDocId",
      JSON.stringify(parseIdArray(formData.removeBusinessRegistrationDocId))
    );
    payload.append(
      "removeFeaturedImageId",
      JSON.stringify(parseIdArray(formData.removeFeaturedImageId))
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      ToastService.success(
        response.data?.responseMessage?.[0] || "Business profile updated!"
      );
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <form
        ...
        className="grid grid-cols-12 gap-4 sm:gap-6 md:gap-8"
      >
        {/* ✅ Business Name */}
        <div className="col-span-12 lg:col-span-6">
          <label className="text-[14px] font-bold font-manrop text-brown-A43">
            Business Name :
          </label>
          <input
            type="text"
        ...
        <PhotoProvider>
          ...
          <div className="col-span-12 lg:col-span-6 flex flex-col gap-4">
            <FileUploadField
              ...
              setFiles={(files) =>
                setFormData((p) => ({ ...p, businessPhotos: files }))
              }
              maxFiles={Math.max(
                0,
                4 -
                  ((businessProfile?.businessPhotos?.length || 0) -
                    (formData.removeBusinessPhotosIds?.length || 0))
              )}
            />
            {businessProfile?.businessPhotos?.length > 0 && (
                ...
                {businessProfile.businessPhotos.map((img, i) => (
                  <div key={i} className="relative flex flex-col gap-4">
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleRemoveImage("businessPhotos", img._id)
                      }
                    >
                      <RxCross1 size={12} />
                    </button>
          ...
          {/* <div className="col-span-12 lg:col-span-6 flex flex-col gap-4">
            <FileUploadField
              ...
              setFiles={(files) =>
                setFormData((p) => ({ ...p, businessNICPhoto: files }))
              }
              maxFiles={Math.max(
                0,
                2 -
                  ((businessProfile?.businessNICPhoto?.length || 0) -
                    (formData.removeBusinessNICPhotoIds?.length || 0))
              )}
            />
            {businessProfile?.businessNICPhoto?.length > 0 && (
              <div className="flex gap-3 mt-3 flex-wrap">
                {businessProfile.businessNICPhoto.map((img, i) => (
                  <div key={i} className="relative flex flex-col gap-4">
                    ...
                    <button
                      ...
                      className="absolute top-[-10px] right-[-10px] bg-red-500 rounded-full p-2 text-background"
                      onClick={() =>
                        handleRemoveImage("businessNICPhoto", img._id)
                      }
                    >
                      <RxCross1 size={12} />
                    </button>
          ...
          {/* <div className="col-span-12 lg:col-span-6 flex flex-col gap-4">
            <FileUploadField
              ...
              setFiles={(files) =>
                setFormData((p) => ({ ...p, businessRegistrationDoc: files }))
              }
              maxFiles={Math.max(
                0,
                1 -
                  ((businessProfile?.businessRegistrationDoc ? 1 : 0) -
                    (formData.removeBusinessRegistrationDocId?.length || 0))
              )}
            />
              ...
              <div className="relative w-max">
                ...
                <button
                  ...
                  onClick={() =>
                    handleRemoveImage(
                      "businessRegistrationDoc",
                      businessProfile.businessRegistrationDoc._id
                    )
                  }
                >
                  <RxCross1 size={12} />
          ...
          <div className="col-span-12 lg:col-span-6 flex flex-col gap-4">
            <FileUploadField
              ...
              setFiles={(files) =>
                setFormData((p) => ({ ...p, businessFeaturedImage: files }))
              }
              maxFiles={Math.max(
                0,
                1 -
                  ((businessProfile?.featuredImage ? 1 : 0) -
                    (formData.removeFeaturedImageId?.length || 0))
              )}
            />
              ...
              <div className="relative w-max">
                ...
                <button
                  ...
                  onClick={() =>
                    handleRemoveImage(
                      "businessFeaturedImage",
                      businessProfile.featuredImage._id
                    )
                  }
                >
                  <RxCross1 size={12} />
