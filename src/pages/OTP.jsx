...
const OTP = () => {
    ...
    const handlePaste = (e) => {
        ...
        const focusIndex = nextEmptyIndex !== -1 ? nextEmptyIndex : Math.min(digits.length - 1, otpValues.length - 1);
        ...
        <div className='bg-background'>
            <div className='flex flex-col md:flex-row 2xl:gap-x-[140px] gap-x-[30px] mx-0 px-0'>
                <div className='w-full md:w-1/2 xl:w-1/3 md:py-[170px] py-[60px] px-[1rem] sm:px-[20px] md:pl-[60px] xl:pl-[120px] flex flex-col gap-10 justify-center'>
                    <h1 className='text-[30px] font-bold font-manrope text-black text-center sm:text-left'>Verify Pin </h1>
                    ...
                    <form className='flex flex-col gap-8' onSubmit={(e) => {
                        ...
                        <div className='flex  justify-between items-center gap-6'>
                                ...
                                {otpValues.map((value, index) => (
                                    <div key={index}>
                                        <label htmlFor={`code-${index + 1}`} className="sr-only">
                                            {index === 0 ? 'First' : index === 1 ? 'Second' : index === 2 ? 'Third' : 'Fourth'} code
                                        </label>
                                        <input
                            ...
                            <div className="flex flex-col items-end">
                                <p className={`text-[16px] font-poppins font-medium ${timeLeft === 0 ? 'text-red-500' : 'text-black'
                                    }`}>
                                    {formatTime(timeLeft)}
                                </p>
                    ...
                    <div className='flex flex-col    gap-[10px]'>
                        ...
                        {timeLeft === 0 && (
                            ...
                            >
                                ...
                                <p className='text-blueB8 text-[15px] font-poppins font-semibold'>Didn't receive the code? </p> <p className='hover:-translate-y-[2px] text-brown-A43 font-semibold font-poppins cursor-pointer'>Resend Code</p>

                            </div>
                        )}
                        <div className='flex gap-[6px] justify-center items-center'>
                            <p className='text-blueB8 text-[15px] font-poppins font-semibold'>Don't have an account? <Link to="/signup-client" className='hover:-translate-y-[2px] text-brown-A43 font-semibold font-poppins'>SignIn</Link></p>
                        </div>
                    </div>
